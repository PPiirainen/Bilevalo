...
#define LED_PIN   2  //Pin for the pixel strand. Can be analog or digital.
...
#define AUDIO_PIN A0  //Pin for the envelope of the sound detector
 #define KNOB_PIN  A6  //Pin for the trimpot 10K
#define BUTTON_1  3   //Button 1 cycles color palettes
#define BUTTON_2  4   //Button 2 cycles visualization modes
#define BUTTON_3  6   //Button 3 toggles shuffle mode (automated changing of color and visual)
...
uint16_t thresholds[] = {764,  1019, 764, 764, 764, 1274 , 1275 ,765 ,1529}; 
...
uint8_t Pace = 30;   // time to edjust the pace of the visuals
...
void rainbowCycle(uint8_t wait) {
  ...
  for(j=0; j<256*50; j++) { // 5 cycles of all colors on wheel
...
if (x == 0) {
      ...
      strand.setPixelColor(hourval, strand.Color(0, 100, 255));
      strand.setPixelColor(hourval - 1, strand.Color(0, 190, 255));
      strand.setPixelColor(hourval + 1, strand.Color(0, 100, 255));
      strand.setPixelColor(minuteval, strand.Color(255, 0, 150));
    
      strand.setPixelColor(secondval, strand.Color(50, 255, 0));
      //strand.setPixelColor(secondval -2, strand.Color(170, 5, 40));
      //strand.setPixelColor(secondval -4, strand.Color(70, 2,20));
      //strand.setPixelColor(secondval -6, strand.Color(20, 0,10));
       
      strand.show();
      strand.setPixelColor(hourval, strand.Color(0, 0, 0));
      strand.setPixelColor(hourval - 1, strand.Color(0, 0, 0));
      strand.setPixelColor(hourval + 1, strand.Color(0, 0, 0));
      strand.setPixelColor(hourval - 2, strand.Color(0, 0, 0));
      strand.setPixelColor(hourval + 2, strand.Color(0, 0, 0));
      strand.setPixelColor(minuteval, strand.Color(0, 0, 0));
      strand.setPixelColor(secondval -2, strand.Color(0, 0, 0));
      strand.setPixelColor(secondval -4, strand.Color(0, 0, 0));
      strand.setPixelColor(secondval -6
      , strand.Color(0, 0, 0));
      strand.setPixelColor(minuteval - 1, strand.Color(0, 0, 0));
      strand.setPixelColor(minuteval + 1, strand.Color(0, 0, 0));
      strand.setPixelColor(secondval, strand.Color(0, 0, 0));
      ...
      hourval = (hourval * 60 + minuteval) / 12;
fade(0.95);
      strand.setPixelColor(hourval, strand.Color(0, 255, 0));
      strand.setPixelColor(hourval - 1, strand.Color(0, 190, 0));
      strand.setPixelColor(hourval + 1, strand.Color(0, 190, 0));
      strand.setPixelColor(minuteval, strand.Color(255, 0, 0));
    
      strand.setPixelColor(secondval, strand.Color(0, 0, 255));
      //strand.setPixelColor(secondval -2, strand.Color(170, 5, 40));
      //strand.setPixelColor(secondval -4, strand.Color(70, 2,20));
      //strand.setPixelColor(secondval -6, strand.Color(20, 0,10));
...
//      strand.setPixelColor(secondval, strand.Color(0, 0, 0));
  ...
  volume = analogRead(AUDIO_PIN);       //Record the volume level from the sound detector
   knob = 1 ;  //Record how far the trimpot is twisted
...
//  ToggleShuffle(); //Toggles shuffle mode. Delete this if you didn't use buttons.
  ...
  delay(Pace);     //Paces visuals so they aren't too fast to be enjoyable
}
    }
...
uint32_t ColorPalette(float num) {
  switch (palette) {
    case 0: return (num < 0) ? Buutti(gradient) : Buutti(num);
    ...
    case 6: return (num < 0) ? Neo(gradient) : Neo(num);
    case 7: return (num < 0) ? USA(gradient) : USA(num);
    case 8: return (num < 0) ? Rainbow(gradient) : Rainbow(num);
    default: return Buutti(gradient);
  }
}
...
void Pulse() {
  ...
  if (volume > 0) {
    ...
    for (int i = start; i < finish; i++) {
      ...
      // Appropriatley adjust the brightness of this pixel using location, volume,
...
void Traffic() {
  ...
  if (volume > 0) {
    ...
    for (int i = 0; i < sizeof(pos); i++) {
      ...
      //Odds will reach -2 by subtraction, but if an even dot goes beyond the LED strand, it'll be purged.
...
void Snake() {
  ...
  fade(0.999); //Leave a trail behind the dot.
...
void Glitter() {
  ...
  //This visual also fits a whole palette on the entire strand
...
void Paintball() {
  ...
  if (bump) {
    ...
    //Pick a random spot on the strand. Random was already reseeded above, so no real need to do it again.
    dotPos = random(strand.numPixels() - 1);
...
//  if (shuffle && millis() / 1000.0 - shuffleTime > 5 && gradient % 2) {
//
...
//  }
  ...
  if (shuffle && millis() / 1000.0 - shuffleTime > 60 && !(gradient % 2)) {
...
uint32_t Buutti(unsigned int i) {
  if (i > 764) return Buutti(i % 765);
  if (i > 609) return strand.Color(90, 0, 255);      //blue
  if (i > 255) return strand.Color(255, 0, 190);  //bbuuttienemmänturkoosi
  return strand.Color(10, 90, 255);                   //buuttiturkoosi
}
...
uint32_t Sunset(unsigned int i) {
  ...
  if (i > 764) return strand.Color((2*i % 255), 0, 255 - (i % 255));          //blue -> red
...
uint32_t Neo(unsigned int i) {
  if (i > 1274) return Neo(i % 1275);  
  if (i > 1019) return strand.Color(255, 255,0);         //flash of yellow
  if (i > 764) return strand.Color(255, 0, 255) ;                     //magneta
  if (i > 509) return strand.Color(255, 10, 148);                     //deeppink
  if (i > 255) return strand.Color(0, 255, i % 255);                 //cyan -> 
  return strand.Color(0, i, 255);                                     //Blue -> cyan
}
